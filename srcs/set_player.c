...
void	get_and_check_texture(t_list *config)
{
	if (!(config->texture[0].img = mlx_xpm_file_to_image(config->data.mlx_ptr,
		config->north_t, &(config->texWidth),
		&(config->texHeight))))
		printf("Error\nTexture nord incorrecte\n");
	if (!(config->texture[1].img = mlx_xpm_file_to_image(config->data.mlx_ptr,
		config->south_t, &(config->texWidth),
		&(config->texHeight))))
		printf("Error\nTexture sud incorrecte\n");
	if (!(config->texture[2].img = mlx_xpm_file_to_image(config->data.mlx_ptr,
		config->west_t, &(config->texWidth),
		&(config->texHeight))))
		printf("Error\nTexture ouest incorrecte\n");
	if (!(config->texture[3].img = mlx_xpm_file_to_image(config->data.mlx_ptr,
		config->east_t, &(config->texWidth),
		&(config->texHeight))))
...
void	check_hit(t_list *config, char **map)
{
	while (config->hit == 0)
	{
		...
		if (map[config->mapY][config->mapY] > 0) 
			config->hit = 1;
	}
}
...
void	texture_calculation(t_list *config, char **map)
{
	if (config->side == 0)
		...
		config->wallX = config->posY + config->perpWallDist * config->rayDirX;
	config->wallX -= floor(config->wallX);
	config->texX = (int)config->wallX * config->texWidth;
	if (config->side == 0 && config->rayDirX > 0)
		config->texX = config->texWidth - config->texX - 1;
	if (config->side == 1 && config->rayDirY < 0)
		config->texX = config->texWidth - config->texX - 1;
}
...
void	draw_text(t_list *config, char **map, int x, int y)
{
	...
	config->step = 1.0 * config->texHeight / config->lineHeight;
	...
	{
		config->texY = (int)config->texPos & (config->texHeight - 1);
