...
void	init_mlx(t_list *config, char **map)
{
	...
	config->data.addr = mlx_get_data_addr(config->data.img, &config->data.bits_per_pixel,
	...
	config->data.addr2 = mlx_get_data_addr(config->data.img2, &config->data.bits_per_pixel,
						&config->data.line_length, &config->data.endian);
